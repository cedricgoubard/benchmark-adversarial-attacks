...
```{python}
 with open(Config.DATA_PATH+'/efficiency_random_padding/random_padding_efficiency_Mnist', 'rb') as f:
    SR_FGSM_dic_with_padding_Mnist=pickle.Unpickler(f).load()
    SR_FGSM_dic_without_padding_Mnist=pickle.Unpickler(f).load()
```

```{python}
 with open(Config.DATA_PATH+'/efficiency_random_padding/random_padding_efficiency_Cifar', 'rb') as f:
...
for c in [0,0.1,0.3,0.5,0.7,0.9]:
    with open(Config.DATA_PATH+'efficiency_adv_trained/Mnist/c='+str(c), 'rb') as f:
...
plots_Cifar={}
for c in [0,0.1,0.3,0.5,0.7,0.9]:
    with open(Config.DATA_PATH+'efficiency_adv_trained/Cifar/c='+str(c), 'rb') as f:
...
```{python}
with open(Config.DATA_PATH+'efficiency_denoiser/denosier_efficiency_white_box_Mnist', 'rb') as f:
    SR_FGSM_dic_without_denoiser_Mnist=pickle.Unpickler(f).load()
    SR_FGSM_dic_with_denoiser_Mnist=pickle.Unpickler(f).load()
```

```{python}
with open(Config.DATA_PATH+'efficiency_denoiser/denosier_efficiency_white_box_Cifar', 'rb') as f:
...
```{python}
with open(Config.DATA_PATH+'perfs_models_without_defense/perfs_global_model_effnet_Mnist', 'rb') as f:
...
```{python}
with open(Config.DATA_PATH+'perfs_models_without_defense/perfs_global_model_effnet_Cifar', 'rb') as f:
...
```{python}
with open(Config.DATA_PATH+'/efficiency_random_padding/random_padding_efficiency_black_boxMnist', 'rb') as f:
        degree_of_change_without_padding_black_box_Mnist=pickle.Unpickler(f).load()
        degree_of_change_with_padding_black_box_Mnist=pickle.Unpickler(f).load()
```

```{python}
with open(Config.DATA_PATH+'/efficiency_random_padding/random_padding_efficiency_black_boxCifar', 'rb') as f:
        degree_of_change_without_padding_black_box_Cifar=pickle.Unpickler(f).load()
        degree_of_change_with_padding_black_box_Cifar=pickle.Unpickler(f).load()
```

### Adversarial training

```{python}
with open(Config.DATA_PATH+'efficiency_adv_trained/black_boxMnist', 'rb') as f:
    degree_of_change_without_adv_training_black_blox_Mnist=pickle.Unpickler(f).load()
    degree_of_change_with_adv_training_black_blox_Mnist=pickle.Unpickler(f).load()
```

```{python}
with open(Config.DATA_PATH+'efficiency_adv_trained/black_boxCifar', 'rb') as f:
    degree_of_change_without_adv_training_black_blox_Cifar=pickle.Unpickler(f).load()
    degree_of_change_with_adv_training_black_blox_Cifar=pickle.Unpickler(f).load()
```

### Denoiser

```{python}
 with open(Config.DATA_PATH+'efficiency_denoiser/denosier_efficiency_black_box_Mnist', 'rb') as f:
...
```{python}
 with open(Config.DATA_PATH+'efficiency_denoiser/denosier_efficiency_black_box_Cifar', 'rb') as f:
