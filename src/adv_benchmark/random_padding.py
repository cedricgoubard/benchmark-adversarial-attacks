...
class ResizePad(tf.keras.layers.Layer):
    """
    define a new tensorflow layer that takes and image (32 by 32) and randomly resize it (less than
    32*32) and randomly pad it back to a 32 by 32 image
    """

    def __init__(self):
        super().__init__()

    def resize_and_pad(self, image):  # pylint: disable=missing-function-docstring, no-self-use
        ...
        paddings = tf.constant(
            [
                [0, 0],
                [top_offset, 32 - top_offset - new_size],
                [left_offset, 32 - left_offset - new_size],
                [0, 0],
            ]
        )
        image = tf.image.resize(
            image, [new_size, new_size], method="nearest", preserve_aspect_ratio=True
        )
